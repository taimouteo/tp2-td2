...
char** keysPredictRun(struct keysPredict* kt, char* partialWord, int* wordsCount) {
	...
	los strings que hay a partir del puntero pasado por parametro, devolviendo el puntero al primer elemento. Aclaración: agregamos una modificación con la variable
	aux pues no funcionaba el caso único donde el prefijo es una palabra de la estructura (una palabra es prefijo de si mismo).
	*/
	...
	*wordsCount = 0;
	...
	// Puntero apunta a la ultima letra del prefijo
	for(int i=0; partialWord[i]!=0; i++) {
		puntero = findNodeInLevel(&puntero, partialWord[i]);
		if(puntero==0) {
			return 0;
		}
		if(partialWord[i+1]==0 && puntero->end==1) {
			aux = puntero;
			(*wordsCount)++;
		}
		puntero = puntero->down;
	...
	*wordsCount = (*wordsCount) + keysPredictCountWordAux(puntero);
	
	// Arreglo de strings.
	char** palabras = (char**)malloc(sizeof(char*)*(*wordsCount));
	if(aux!=0) {
		palabras[0] = strDup(aux->word);
		palabras++;
...
char** keysPredictListAll(struct keysPredict* kt, int* wordsCount) {
	...
	el primer nodo de la estructura kt, listando asi la estructura completa.
	*/
...
void keysPredictDeleteAux(struct node* n) {
	/*
	Funcion auxiliar recursiva que toma un puntero, y en caso de ser no nulo, libera la memoria del nodo actual. Previamente,
